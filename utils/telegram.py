...
⏰ <b>Time:</b> {datetime.now().strftime('%H:%M:%S')}
    ...
    def send_position_closed(self, symbol: str, side: str, size: float, entry_price: float, close_price: float, pnl: float, reason: str, strategy: str, hold_duration: str, portfolio_summary: Dict) -> bool:
        # Fix profit/loss logic for SELL trades
        if side.lower() == 'sell':
            # For SELL trades: price decrease = WIN, price increase = LOSS
            price_change = entry_price - close_price  # Positive if price went down (WIN)
            is_win = price_change > 0  # WIN if price decreased
        else:
            # For BUY trades: price increase = WIN, price decrease = LOSS
            price_change = close_price - entry_price  # Positive if price went up (WIN)
            is_win = price_change > 0  # WIN if price increased
        
        # Use correct emoji based on actual win/loss
        profit_emoji = "💚" if is_win else "❤️" if not is_win else "💛"
